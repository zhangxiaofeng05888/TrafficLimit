 ...
 */
angular.module('app').controller('deepParkingCtl', function ($scope) {
    ...
    var initDeepData = function () {
        ...
        if ($scope.poi.parkings.length === 0) { // 增加停车场默认为空的情况
            $scope.poi.parkings[0] = new FM.dataApi.IxPoiParking({ pid: objectCtrl.data.pid, parkingType: '' }); // parkingType要有默认值，因为常规POI深度信息保存时会有不为空的判断
        }
    };
    initDeepData();
...
//        Utils.setCheckboxMutex(event,obj,rejectVal);
    ...
    $scope.avgTotalNum = function () {
        var totalNum = $scope.poi.parkings[0].totalNum;
        if (!totalNum) {
            $scope.poi.parkings[0].totalNum = 0;
        }
    };
});
