angular.module('lazymodule', []).controller('DateCtrl2', ['$scope', '$timeout', '$compile', function ($scope, $timeout, $compile) {
    /* 星期*/
    ...
    /* 外部传递str*/
    $scope.$on('set-code-2', function (event, data) {
        if (data) {
            $scope.dateList = [];
    ...
    /* 判断功能按钮状态*/
    $scope.$on('btn-control-2', function (event, data) {
        if (data) {
                ...
                if (weeksStr.length >= 1 && weeksStr[0].indexOf(')(') == -1 && mutiDate.length == 1 && mutiDate[0].indexOf('y') == -1 && mutiDate[0].indexOf('{d') == -1) {
                    ...
                    if (weeksArr[0].split('')[0] == '(' && weeksArr[0].split('')[weeksArr[0].split('').length - 1] == ')' && weeksArr[0].indexOf(']*[') > -1) { // 如果是持续时间 从...
                        ...
                        if (weeksArr[0].indexOf(')(M') > -1) { // (M12d1)(M12d31)]*[(h23m0)(h23m59)
                            ...
                            var _arr = temp.split(')]*[(');
                            detail = '从' + $scope.timeAnalyze(_arr[0].split(')(')[0]) + '的' + $scope.timeAnalyze(_arr[1].split(')(')[0]) + '到' + $scope.timeAnalyze(_arr[0].split(')(')[1]) + '的' + $scope.timeAnalyze(_arr[1].split(')(')[1]);
                        }
    ...
    $scope.removeList = function (e, index) {
        ...
        $scope.$emit('get-date-2', $scope.dateString);
    ...
    $scope.dateSave = function (e) {
                        ...
                        case 'd':
                            ...
                            if ($scope.vagueTime) {
                                ...
                                if (yearLength == 0) { // 开始年和结束年为同一年
                                    ...
                                    return '[[[(y' + $beginYear + 'M' + $beginMonth + 'd' + $beginDay + 'h' + $beginHour + 'm' + $beginMinute + ')' +
                                        '(y' + $beginYear + 'M12d31h23m59)]' + difYear + '+[(y' + $endYear + 'M1d1h0m0)' +
                                        '(y' + $endYear + 'M' + $endMonth + 'd' + $endDay + 'h' + $endHour + 'm' + $endMinute + ')]]*z]';
                                }
        ...
        if ($scope.dateValid(dateWell)) {
            ...
            $.each($scope.dateList, function (i, v) {
                if (i != $scope.dateList.length - 1) allStr += v.code + '+';
                else allStr += v.code;
            });
