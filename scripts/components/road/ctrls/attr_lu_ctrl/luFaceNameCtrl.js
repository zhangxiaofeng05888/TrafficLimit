...
angular.module('app').controller('LuFaceNameCtl', ['$scope', 'dsEdit', 'dsMeta', '$timeout', function ($scope, dsEdit, dsMeta, $timeout) {
    var objCtrl = fastmap.uikit.ObjectEditController();
    $scope.luFaceNames = objCtrl.namesInfos;
    $scope.namesGroup = objCtrl.currentGroup;
    $scope.selectedLangcodeArr = [];
    var timer;
    var getSelectedLangcode = function () {
        $scope.selectedLangcodeArr.length = 0;
        for (var k in $scope.namesGroup) {
            if ($scope.selectedLangcodeArr.indexOf($scope.namesGroup[k].langCode) < 0) {
                $scope.selectedLangcodeArr.push($scope.namesGroup[k].langCode);
            }
        }
    };
    getSelectedLangcode();
    ...
    function translateFunc(nameInfo) {
        // 标识多音字是否已经选完
        var flag = true;
        for (var i = 0; i < $scope.namesGroup[0].phoneticArr.length; i++) {
            if ($scope.namesGroup[0].phoneticArr[i].length > 1) {
                flag = false;
                break;
            }
        }
        ...
        var nParam = {
            word: $scope.namesGroup[0].name,
            languageType: 'eng',
            pinyin: $scope.namesGroup[0].phoneticArr.join(' ')
        };
    ...
    $scope.changeNameSource = function (nameInfo) {
        if ($scope.luFaceNames.srcFlag == '1') {
            translateFunc(nameInfo);
        }
    };
    /* 名称语音*/
    $scope.namePronunciation = function (nameCn, nameInfo) {
        $scope.luFaceNames.name = Utils.ToDBC(nameCn);
        if (timer) {
        ...
        timer = $timeout(function () {
            if (!nameCn) {
                if (nameInfo.langCode == 'CHI' || nameInfo.langCode == 'CHT') {
                    for (i = 0; i < $scope.namesGroup.length; i++) {
                        $scope.namesGroup[i].name = '';
                        $scope.namesGroup[i].phonetic = '';
                        $scope.namesGroup[i].phoneticArr = [];
                    }
                }
                return;
            }
            ...
            dsMeta.getPyPolyphoneConvert(param).then(function (data) {
                if (data.errcode == 0) {
                    ...
                    // 如果有英文 则翻译
                    for (i = 0; i < $scope.namesGroup.length; i++) {
                        if ($scope.namesGroup[i].langCode == 'ENG') {
                            translateFunc($scope.namesGroup[i]);
                        }
                    }
                } else {
                    nameInfo.phonetic = '';
    ...
    $scope.selectPolyphonic = function (event, index, parentIndex) {
        $scope.luFaceNames.phoneticArr[parentIndex] = [event.target.innerText];
        // 选择多音字后，需同步刷新英文
        for (var i = 0; i < $scope.namesGroup.length; i++) {
            if ($scope.namesGroup[i].langCode == 'ENG') {
                translateFunc($scope.namesGroup[i]);
            }
        }
    ...
    $scope.langCodeChange = function (langCode) {
        ...
        if (langCode != 'CHI' && langCode != 'CHT') {
            ...
            $scope.selectedLangcodeArr = [];
            for (var j = 0; j < $scope.langCodeOptions.length; j++) {
                if ($scope.langCodeOptions[j].id != 'CHI') {
                    $scope.selectedLangcodeArr.push($scope.langCodeOptions[j].id);
                }
            }
        }
    ...
    $scope.refreshNameLangCode = function () {
        $scope.namesGroup.sort(function (a, b) {
