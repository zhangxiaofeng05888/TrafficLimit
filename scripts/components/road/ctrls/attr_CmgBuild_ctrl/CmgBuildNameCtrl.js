/**
 * Created by mali on 2017/5/24.
 */
angular.module('app').controller('CmgBuildNameCtrl', ['$scope', 'dsEdit', 'dsMeta', '$timeout', function ($scope, dsEdit, dsMeta, $timeout) {
    var objCtrl = fastmap.uikit.ObjectEditController();
    $scope.cmgBuildNames = objCtrl.namesInfos; //
    $scope.cmgBuildNameGroup = objCtrl.currentGroup;
    $scope.selectedLangcodeArr = [];
    var timer;
    var getSelectedLangcode = function () {
        $scope.selectedLangcodeArr.length = 0;
        for (var k in $scope.cmgBuildNameGroup) {
            if ($scope.selectedLangcodeArr.indexOf($scope.cmgBuildNameGroup[k].langCode) < 0) {
                $scope.selectedLangcodeArr.push($scope.cmgBuildNameGroup[k].langCode);
            }
        }
    };
    getSelectedLangcode();
    // 转英文
    function translateFunc(nameInfo, cnfield, phonefield) {
        var currentName = nameInfo;
        var nParam = {
            word: nameInfo[cnfield],
            languageType: 'eng'
        };
        dsMeta.nameTranslate(nParam).then(function (nData) {
            if (nData.errcode == 0) {
                currentName[cnfield] = nData.data.eng;
                currentName[phonefield] = '';
            } else {
    ...
    $scope.changeNameSource = function (nameInfo) {
        if ($scope.cmgBuildNames.srcFlag == '1') {
    ...
    /* 名称语音*/
    $scope.namePronunciation = function (nameCn, nameInfo, cnfield, phonefield, phoneticArr) {
        $scope.cmgBuildNames[cnfield] = Utils.ToDBC(nameCn);
        if (timer) {
        ...
        timer = $timeout(function () {
            if (!nameCn) {
                if (nameInfo.langCode == 'CHI' || nameInfo.langCode == 'CHT') {
                    for (i = 0; i < $scope.cmgBuildNameGroup.length; i++) {
                        $scope.cmgBuildNameGroup[i][cnfield] = '';
                        $scope.cmgBuildNameGroup[i][phonefield] = '';
                        $scope.cmgBuildNameGroup[i][phoneticArr] = [];
                    }
                }
                return;
            }
            ...
            dsMeta.getPyPolyphoneConvert(param).then(function (data) {
                if (data.errcode == 0) {
                    nameInfo[phonefield] = data.data.phonetic;
                    var phoneticStr = [];
                    var tempArr = [];
                    ...
                    // 如果有英文 则翻译
                    for (i = 0; i < $scope.cmgBuildNameGroup.length; i++) {
                        if ($scope.cmgBuildNameGroup[i].langCode == 'ENG') {
                            // 经与玉秀确认cmgfeature 英文不用程序处理
                            // translateFunc($scope.cmgBuildNameGroup[i], cnfield, phonefield);
                        }
                    }
                } else {
                    nameInfo[phonefield] = '';
    ...
    // 选择多音字
    $scope.selectPolyphonic = function (event, index, parentIndex, phoneticArr) {
        $scope.cmgBuildNames[phoneticArr][parentIndex] = [event.target.innerText];
        console.log(event.target + index + '--' + parentIndex);
    };
    // 代码语言字段切换时，判断语言不能重复
    ...
    $scope.changeLangCode = function (nameInfo) {
        if (nameInfo.langCode == 'ENG') {
            var nParam = {
                word: $scope.cmgBuildNameGroup[0].name,
                languageType: 'eng'
            };
            dsMeta.nameTranslate(nParam).then(function (data) {
                if (data.errcode == 0) {
                    $scope.cmgBuildNames.name = data.data.eng;
                    $scope.cmgBuildNames.phonetic = '';
                } else {
                    swal('查找失败', '可能是服务出错或者输入过长，请重新尝试', 'error');
                }
            });
    ...
    $scope.refreshNameLangCode = function () {
        $scope.cmgBuildNameGroup.sort(function (a, b) {
