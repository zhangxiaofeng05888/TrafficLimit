/**
 * Created by wangmingdong on 2017/8/21.
 * 警示信息面板
 */
angular.module('app').controller('linkWarningCtl', ['$scope', '$timeout', 'dsEdit', 'appPath', '$ocLazyLoad', function ($scope, $timeout, dsEdit, appPath, $ocLazyLoad) {
    ...
    $ocLazyLoad.load(appPath.road + 'ctrls/attr_linkWarning_ctrl/carTypeCtrl.js');
    // 初始化函数;
    ...
    $scope.selectPicCode = function (code) {
        var flag = false;
        dsEdit.getByPid($scope.rdLinkWarningObj.linkPid, 'RDLINK').then(function (ret) {
            if (ret) {
                if (ret.forms.length > 0) {
                    ...
                    $scope.rdLinkWarningObj.typeCode = code.substr(0, 5);
                    $scope.typeCodeImg = $scope.rdLinkWarningObj.typeCode.substr(0, 5);
    ...
    $scope.showPicSelect = function () {
        ...
        timer = $timeout(function () {
            ...
            var typeCode = $scope.rdLinkWarningObj.typeCode;
            if (typeCode == '') {
                $scope.showImgData = false;
                return;
            }
    ...
    $scope.verifyNumber = function (t, min, max, model) {
        ...
        if (value < min) {
            $scope.rdLinkWarningObj[model] = min;
            return;
        }
        if (value > max) {
            $scope.rdLinkWarningObj[model] = max;
            return;
        }
        value = parseInt(value.replace(/\D/g, ''), 10);
        $scope.rdLinkWarningObj[model] = value;
    };
    ...
    function timeoutLoad() {
        $timeout(function () {
            $ocLazyLoad.load(appPath.tool + 'fmTimeComponent/fmdateTimer.js').then(function () {
                ...
                $ocLazyLoad.load(appPath.road + 'ctrls/attr_linkWarning_ctrl/carTypeCtrl.js').then(function () {
                    $scope.carPopoverURL = appPath.road + 'attr_linkWarning_tpl/carTypeTpl.html';
                });

                $timeout(function () {
                    $scope.$on('get-date', function (event, data) {
                        $scope.rdLinkWarningObj.timeDomain = data;
                    });
                    $scope.$broadcast('set-code', $scope.rdLinkWarningObj.timeDomain);
                }, 100);
            });
        });
    }
    ...
    $scope.initializeData = function () {
        ...
        $scope.rdLinkWarningObj = objCtrl.data;
