    ...
    function ($scope, $ocLazyLoad, appPath, $interval, dsMeta) {
        ...
        var getFix = function (langCode) {
            dsMeta.searchFix({ langCode: langCode }).then(function (data) {
                if (data) {
                    if (!$scope.flagOfEdit) {
                        data.suffix.unshift({ id: '', label: '' });
                        data.infix.unshift({ id: '', label: '' });
                    } else {
        ...
        $scope.initializeData = function () {
            ...
            $scope.roadNameData.hwInfoFlag = null;
        ...
        $scope.selectVal = function (row, index, type) {
            var param = {};
            if (type == 'admin') {
                $scope.roadNameData.adminId = parseInt(row.adminareacode, 10);
        ...
        $scope.doSave = function () {
            validateForm($scope.RoadNameEditPanelForm);
            if ($scope.RoadNameEditPanelForm.$invalid) {
