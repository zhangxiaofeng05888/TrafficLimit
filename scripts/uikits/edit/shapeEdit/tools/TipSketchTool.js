...
fastmap.uikit.shapeEdit.TipSketchTool = fastmap.uikit.shapeEdit.ShapeTool.extend({
    initialize: function () {
    ...
    drawRuntimeGeometry: function () {
        var lineSymbol = this.symbolFactory.getSymbol('sketch_line');
    ...
    onKeyUp: function (event) {
        ...
        switch (key) {
            case 'd':   // 清空草图
                ...
                newEditResult.finalGeometry = null;
                this.shapeEditor.createOperation('修改经过线', newEditResult);
                break;
            default:
                break;
        }
    ...
    onLeftButtonUp: function (event) {
        ...
        if (this.runTimeLineArr.length >= 5) {
            ...
            newEditResult.startNode = null;
            this.shapeEditor.createOperation('如果第一次点了没画怎不创建起点', newEditResult);
        }
        this.runTimeLineArr = [];
        return true;
    }
