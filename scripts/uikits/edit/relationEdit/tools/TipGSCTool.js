...
fastmap.uikit.relationEdit.TipGSCTool = fastmap.uikit.relationEdit.RelationTool.extend({
    initialize: function () {
    ...
    getCrossPoints: function (features, selectBox) {
        ...
        for (i = 0; i < features.length; i++) {
            if (features[i].geometry.type === 'GeometryCollection') {
                segments = this.uikitUtil.splitToSegments(features[i].geometry.geometries[1]);
            } else {
    ...
    onLeftButtonClick: function (event) {
        ...
        if (!(this.editResult.operation == 'SelectCross' || this.editResult.operation == 'SortLink')) {
            return false;
        }
    ...
    onSelectCrossPoint: function (index) {
        ...
        for (var i = 0; i < crossLink.length; i++) {
            var geometry = {};
            if (crossLink[i].feature.geometry.type === 'GeometryCollection') { // 对选中的对象为测线的处理
                geometry = crossLink[i].feature.geometry.geometries[1];
            } else {
                geometry = crossLink[i].feature.geometry;
            }
            p = this.uikitUtil.createPoint(geometry.coordinates[0]);
            if (this.uikitUtil.isSamePoint(cp, p)) { // 交点为link起点
                crossLink[i].startEnd = 1;
            } else {
                p = this.uikitUtil.createPoint(geometry.coordinates[geometry.coordinates.length - 1]);
        ...
        // 按照是否是隧道排序，隧道的排在最底层
        crossLink.sort(function (a, b) {
            var fa = a.feature.properties.geoLiveType === 'RDLINK' && a.feature.properties.form.split(';').indexOf('31') >= 0;
            var fb = b.feature.properties.geoLiveType === 'RDLINK' && b.feature.properties.form.split(';').indexOf('31') >= 0;
        ...
        for (i = 1; i < crossLink.length; i++) {
            f31 = crossLink[i].feature.properties.geoLiveType === 'RDLINK' && crossLink[i].feature.properties.form.split(';').indexOf('31') >= 0;
            if (crossLink[i].startEnd === 0 && !f31) { // 不是平交，也不是隧道，则层级增加
                zlevel++;
            }
