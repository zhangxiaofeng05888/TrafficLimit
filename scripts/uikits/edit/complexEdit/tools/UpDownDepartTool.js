...
fastmap.uikit.complexEdit.UpDownDepartTool = fastmap.uikit.complexEdit.ComplexTool.extend({
    initialize: function () {
        fastmap.uikit.complexEdit.ComplexTool.prototype.initialize.call(this);
        // 绑定函数作用域
        FM.Util.bind(this);
        this.eventController = fastmap.uikit.EventController();
        this.name = 'UpDownDepartResult';
        this.snapActor = null;
        this.leftLineObj = {
            type: 'LineString',
            coordinates: []
    ...
    resetMouseInfo: function () {
        ...
        if (this.editResult.links.length == 1 && !this.editResult.distance) {
            this.setMouseInfo('选择挂接的link进行追踪或按c键制作上下线分离!');
            return;
        }
        if (this.editResult.links.length > 1 && !this.editResult.distance) {
            this.setMouseInfo('对所选的link进行修改或按c键制作上下线分离!');
            return;
        }
    ...
    _autoTrail: function (feature, dirNode) {
        var _self = this;
        var newEditResult = FM.Util.clone(this.editResult);
        this.dirNode = this._getConnectNode(feature, 'dirNode');
        newEditResult.sNodePid = (this.dirNode == this.editResult.links[0].properties.snode) ? this.editResult.links[0].properties.enode : this.editResult.links[0].properties.snode;
        newEditResult.distance = undefined;
        var requestParma = {
            command: 'CREATE',
            type: 'RDLINK',
