    ...
    var w5cValidator = angular.module('w5c.validator', ['ng'])
        .provider('w5cValidator', [function () {
            var defaultRules = {
                    ...
                    w5cremotecheck: '该项格式不正确', // 提示语根据情况 在各个模块中重写
                    url: '输入URL格式不正确',
                    max: '该项输入值不能大于{max}',
                    min: '该项输入值不能小于{min}',
                    customizer: '自定义验证不通过'
                ...
                elemTypes = ['text', 'password', 'email', 'number', 'url', 'tel', 'hidden', ['textarea'],
                    ['select'],
                    ['select-multiple'],
...
(function () {
    angular.module('w5c.validator')
                ...
                return {
                    ...
                    controller: [
                        ...
                        function ($scope, $element, $attrs) {
                             ...
                             */
                            this._init = function () {
                                _globalValidatorRules = w5cValidator.rules;
                                ...
                                if ($attrs.w5cFormValidate) {
                                    $scope.$watch($attrs.w5cFormValidate, function (newValue) {
                                        if (newValue) {
                                            _self.options = options = angular.extend({}, w5cValidator.options, newValue.options);
                                        }
                                        if (newValue.rules) {
                                            w5cValidator.setRules(newValue.rules);
                                        }
                                    }, true);
                                }
                            ...
                            $scope.$on('$destroy', function () {
                                w5cValidator.setRules(_globalValidatorRules);
                            });
                        }
                    ],
                    ...
                    var doValidate = function () {
                        ...
                        $http.get(url).success(function (data) {
                            ...
                            if (!state) {
                                ...
                                w5cValidator.removeError(elem,w5cFormCtrl.options); // 存在bug，移除错误提示信息
                            }
                        });
                    };
            ...
            return {
                ...
                link: function (scope, elem, attrs, ctrls) {
                    ...
                    var doValidate = function () {
                        ...
                        var url = attValues.url;
                        $http.get(url).success(function (data) {
                            var state = attValues.compare(data);
                            ngModelCtrl.$setValidity('w5cremotecheck', state);
                            if (!state) {
                                var errorMsg = w5cValidator.getErrorMessage('w5cremotecheck', elem[0]);
                                w5cValidator.showError(elem[0], [errorMsg], w5cFormCtrl.options);
                                if (formCtrl[elem[0].name]) {
                                    formCtrl[elem[0].name].w5cError = true;
                                }
        ...
        .directive('w5cDynamicCustomizer', ['$timeout', '$http', 'w5cValidator', function ($timeout, $http, w5cValidator) {
            return {
                require: ['ngModel', '?^w5cFormValidate', '?^form'],
                link: function (scope, elem, attrs, ctrls) {
                    var ngModelCtrl = ctrls[0],
                    ...
                    var doValidate = function () {
                        var state = scope.$eval(attrs.w5cDynamicCustomizer);
                        ngModelCtrl.$setValidity('w5cDynamicCustomizer', state);
                        if (!state) {
                            var errorMsg = w5cValidator.getErrorMessage('w5cDynamicCustomizer', elem[0]);
                            w5cValidator.showError(elem[0], [errorMsg], w5cFormCtrl.options);
                            if (formCtrl[elem[0].name]) {
                                formCtrl[elem[0].name].w5cError = true;
                            }
                            ...
                            w5cValidator.removeError(elem,w5cFormCtrl.options);
                        }
                    };

                    ngModelCtrl.$viewChangeListeners.push(function () {
                        formCtrl.$errors = [];
                        ngModelCtrl.$setValidity('w5cDynamicCustomizer', true);
                        if (ngModelCtrl.$invalid && !ngModelCtrl.$error.w5cDynamicCustomizer) {
                            return;
                        }
                        if (ngModelCtrl.$dirty) {
                            doValidate();
