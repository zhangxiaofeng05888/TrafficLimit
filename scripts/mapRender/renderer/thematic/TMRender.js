FM.mapApi.render.renderer.TMRender = L.Class.extend({
    initialize: function () {
    ...
    getSymbol: function (feature, zoom) {
        ...
        var featureRender = this._getRenderByTMCode(TMCode, feature, zoom);
        if (!featureRender) {
            throw new Error('未实现符号化:' + TMCode);
        }
    ...
    getHighlightSymbol: function (feature, zoom) {
        ...
        var featureRender = this._getRenderByTMCode(TMCode, feature, zoom);
        if (!featureRender) {
            throw new Error('未实现高亮符号化:' + TMCode);
        }
        return featureRender.getHighlightSymbol();
    },

    _getRenderByTMCode: function (TMCode, feature, zoom) {
