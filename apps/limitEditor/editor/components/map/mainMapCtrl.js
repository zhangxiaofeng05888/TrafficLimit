...
angular.module('app').controller('mainMapCtrl', ['$scope', '$cookies', '$q', '$timeout', '$ocLazyLoad', 'hotkeys',
    ...
    function ($scope, $cookies, $q, $timeout, $ocLazyLoad, hotkeys, dsLazyload, appPath) {
        ...
        var loadMapToolbar = function () {
            ...
            promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/selectToolCtrl.js')); // 专项作业深度信息使用
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/selectTmcCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addSideRoadCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addTmcCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addNodeFeatureCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addLinkFeatureCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addFaceFeatureCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addRdRelationCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addRdBranchCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addCRFShapeCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addSameFeatureCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addPoiCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addAdminPointCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addTipsCtrl.js'));
            // promises.push($ocLazyLoad.load(appPath.root + 'scripts/components/tools/ctrls/toolbar-map/addFeatureShape/addUpDownDepart.js'));
        ...
        var setMapInitialPosition = function (point) {
            var mapCookie = $cookies.getObject(mapCookieKey);
            if (mapCookie) {
                $scope.map.setView(mapCookie.viewPoint, mapCookie.zoom);
            } else if (point) {
                $scope.map.setView([point.coordinates[1], point.coordinates[0]], 12);
            } else { // 随便定位一个位置（todo: 要改成天安门的坐标）
                $scope.map.setView([40.88888, 116.33333], 12);
            }
        };

        var setDefaultScene = function () {
            var taskType = App.Temp.taskType;
            if (taskType === 0 || taskType === 2) { //   type是0、2的子任务，默认是 “POI粗编作业场景”
        ...
        var initialize = function () {
            ...
            layerCtrl = new fastmap.uikit.LayerController({
                config: App.editorLayerConfig
            });

            // 默认使用地图漫游工具
            toolCtrl.resetCurrentTool('PanTool', null, null);

            var point = App.Temp.cityGeometry;
            setMapInitialPosition(point);
                ...
                for (var i = 0; i < editTools.length; i++) {
                    ...
                    if (tool) {
                        tools.push({
                            ...
                            type: editTools[i],
                            class: 'feaf',
                            callback: editToolHanler.handler
                        });
                    }
                }
            }
        ...
        var resetToDefaultScene = function () {
            var taskType = App.Temp.taskType;
            var sceneID = (taskType === 0 || taskType === 2) ? 'POIWorkScene' : 'BaseInfoScene';
